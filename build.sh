...
case "${2}" in
    "ubuntu"|"debian")
      ...
      cp -rf html startup.sh shutdown.sh rule.service dist/rule/usr/local/share/rule
      # 修改权限
    ...
    "centos")
      ...
      go build -ldflags="-s -w" -o rule  # 编译
      ...
      cp -rf dist/rpmbuild ~/
      sudo rpmbuild -ba ~/rpmbuild/SPECS/rule.spec  # rpm安装包文件生成在/root/rpmbuild/RPMS目录下
    ;;
    *)
      echo "错误的'系统'参数"
      exit 0
